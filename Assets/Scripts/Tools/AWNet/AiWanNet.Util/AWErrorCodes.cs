...
{
	public static class AWErrorCodes
	{
		...
		{
			AWErrorCodes.errorsByCode[code] = message;
		}
		public static string GetErrorMessage(int code, Logger log, params object[] args)
		{
			...
			try
			{
				result = string.Format(AWErrorCodes.errorsByCode[code], args);
			}
			catch (Exception ex)
			{
				log.Error(new string[]
				{
					"Formatting error string failed with exception: " + ex.Message
				});
				result = AWErrorCodes.errorsByCode[code];
			}
			return result;
		}
	}
